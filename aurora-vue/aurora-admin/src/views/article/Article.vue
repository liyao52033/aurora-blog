...
export default {
  ...
  methods: {
	...
	uploadFileChange(e) {
		...
		var formdata = new FormData()
		if (file.size / 1024 < this.config.UPLOAD_VIDEO) {
			formdata.append('file', file)
			this.axios.post('/api/admin/articles/images',formdata).then(res => {
				const $vm = this.$refs.md
				var suffix = file.name.substring(file.name.lastIndexOf("."))
				if(suffix == '.mp4' || suffix == '.m3u8'){
				    $vm.insertText($vm.getTextareaDom(),
				    {
				      prefix: `![${file.name}](${res.data.data})`,
				    	subfix: '',
				    	str: ''
				    })
				} else {
			...
			this.$confirm('文件超出范围(最大支持300M)，请上传至网盘后使用上方链接功能')
		  }
	},
    saveArticleDraft() {
      if (this.article.articleTitle.trim() == '') {
